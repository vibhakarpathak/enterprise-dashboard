...
const Card = dynamic(() => import('@enterprise/ui').then((mod) => mod.Card));
const CardHeader = dynamic(() => import('@enterprise/ui').then((mod) => mod.Card.Header));
const CardBody = dynamic(() => import('@enterprise/ui').then((mod) => mod.Card.Body));
  ...
  return (
    <div className="min-h-screen bg-surface text-text selection:bg-primary/30 transition-colors duration-500">
      ...
      <main className="max-w-6xl mx-auto p-8 lg:p-12 space-y-12">
        ...
        <RevenueCard isLoading={isLoading} isError={isError} data={data} />

        <SearchAndVirtualList
          search={search}
          setSearch={setSearch}
...
// ── Revenue Card (fixed for exactOptionalPropertyTypes) ────────
function RevenueCard({
  ...
  isLoading: boolean;
  isError: boolean;
  data?: RevenueWidget | undefined;  // Explicitly allow undefined
}) {
  return (
    <Card>
      <CardHeader>Revenue</CardHeader>
      <CardBody>
        <RevenueContent isLoading={isLoading} isError={isError} data={data} />
      </CardBody>
    </Card>
  );
}
...
function RevenueContent({
  ...
  if (isError) {
    return <p className="text-red-500">Failed to load revenue data</p>;
  }

  return <RevenueSuccessState data={data} />;
}
...
function RevenueSuccessState({ data }: { data?: RevenueWidget | undefined }) {
  return (
    <>
      ...
      <p className="text-sm text-text opacity-70 flex items-center gap-1">
        Trend:
        <span
          className={data?.trend === 'up' ? 'text-green-500' : 'text-red-500'}
        >
          {data?.trend === 'up' ? '↑' : '↓'} {data?.trend ?? 'neutral'}
        </span>
      </p>
    </>
  );
...
function SearchAndVirtualList({
  ...
  return (
    <>
      <Card>
        <CardHeader>Search & Virtual List</CardHeader>
        <CardBody>
          ...
          <input
            id="search-input"
            type="text"
            className="border border-border p-sm w-full rounded-md bg-surface text-text focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
            placeholder="Search 10,000 items..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            aria-label="Search through 10,000 items"
          />
      ...
      <div
        ...
        role="listbox"
        aria-label="Virtualized list of items"
      >
        <div
          style={{
            ...
            {visibleItems.map((item, index) => {
              ...
              return (
                <div
                  key={globalIndex}
                  role="option"
                  className="border-b border-border px-md flex items-center text-text absolute w-full"
                  style={{
                    height: ITEM_HEIGHT,
                    top: `${globalIndex * ITEM_HEIGHT}px`,
                  }}
                >
                  {item}
                </div>
...
function ActionCards() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
      ...
      <Card>
        <CardHeader>UI Component Library</CardHeader>
        <CardBody>
...
function StatsGrid() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <StatCard title="System Health" value="99.9%" sub="Stable" status="green" />
      ...
      <section className="lg:col-span-8 bg-surface border border-border rounded-3xl p-8 shadow-sm">
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            ...
            aria-label="View full activity logs"
          >
            Full Logs →
          </button>
        </div>
            ...
            >
              ...
              <div className="flex-1">
                <p className="font-semibold group-hover:text-primary transition-colors">
                  Updated @enterprise/hooks, @enterprise/tokens, @enterprise/ui
                </p>
